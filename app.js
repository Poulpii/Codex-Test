(function clientApp() {
  ...
  function renderExplorer() {
    ...
    years.forEach((year) => {
      ...
      const yearFolder = treeFolder(`📁 ${year}`);
      ...
      filters().forEach((filter) => {
        ...
        const filterFolder = treeFolder(`📁 ${label(filter)}`);
      ...
      if (archived.length) {
        const archiveFolder = treeFolder('🗄️ Archives');
  ...
  function treeFolder(title) {
    const box = document.createElement('div');
    box.className = 'tree-folder';
    box.innerHTML = `<div class="tree-folder-header"><span class="folder-arrow">▼</span> ${esc(title)}</div><div class="tree-folder-content"></div>`;
